...
if (!isset($GLOBALS['TCA']['fe_user']['ctrl']['type'])) {
    if (file_exists($GLOBALS['TCA']['fe_user']['ctrl']['dynamicConfigFile'])) {
        require_once($GLOBALS['TCA']['fe_user']['ctrl']['dynamicConfigFile']);
    }
    // no type field defined, so we define it here. This will only happen the first time the extension is installed!!
    $GLOBALS['TCA']['fe_user']['ctrl']['type'] = 'tx_extbase_type';
    $tempColumnstx_praxisprojekclaas2_fe_user = [];
    $tempColumnstx_praxisprojekclaas2_fe_user[$GLOBALS['TCA']['fe_user']['ctrl']['type']] = [
        'exclude' => true,
    ...
    \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addTCAcolumns('fe_user', $tempColumnstx_praxisprojekclaas2_fe_user);
}

\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addToAllTCAtypes(
    'fe_user',
    $GLOBALS['TCA']['fe_user']['ctrl']['type'],
    '',
    'after:' . $GLOBALS['TCA']['fe_user']['ctrl']['label']
);
...
\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addTCAcolumns('fe_user',$tmp_praxisprojekclaas2_columns);

/* inherit and extend the show items from the parent class */

if (isset($GLOBALS['TCA']['fe_user']['types']['Tx_Extbase_FrontendUser']['showitem'])) {
    $GLOBALS['TCA']['fe_user']['types']['Tx_Praxisprojekclaas2_User']['showitem'] = $GLOBALS['TCA']['fe_user']['types']['Tx_Extbase_FrontendUser']['showitem'];
} elseif(is_array($GLOBALS['TCA']['fe_user']['types'])) {
    // use first entry in types array
    $fe_user_type_definition = reset($GLOBALS['TCA']['fe_user']['types']);
    $GLOBALS['TCA']['fe_user']['types']['Tx_Praxisprojekclaas2_User']['showitem'] = $fe_user_type_definition['showitem'];
} else {
    $GLOBALS['TCA']['fe_user']['types']['Tx_Praxisprojekclaas2_User']['showitem'] = '';
}
$GLOBALS['TCA']['fe_user']['types']['Tx_Praxisprojekclaas2_User']['showitem'] .= ',--div--;LLL:EXT:praxisprojekclaas2/Resources/Private/Language/locallang_db.xlf:tx_praxisprojekclaas2_domain_model_user,';
$GLOBALS['TCA']['fe_user']['types']['Tx_Praxisprojekclaas2_User']['showitem'] .= 'id, montag, dienstag, mittwoch, donnerstag, freitag, samstag, sonntag';

$GLOBALS['TCA']['fe_user']['columns'][$GLOBALS['TCA']['fe_user']['ctrl']['type']]['config']['items'][] = ['LLL:EXT:praxisprojekclaas2/Resources/Private/Language/locallang_db.xlf:fe_user.tx_extbase_type.Tx_Praxisprojekclaas2_User','Tx_Praxisprojekclaas2_User'];
